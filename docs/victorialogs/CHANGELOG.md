...
* SECURITY: check the `-httpAuth.*` credentials for requests to any path ending with `/config`, `/reload`, `/snapshot` and a few other suffixes. Previously, HTTP Basic Auth was skipped for such requests. For example, `/select/vmalert/-/reload` was proxied to vmalert without checking the `-httpAuth.*` credentials. See [VictoriaMetrics#11548](https://github.com/VictoriaMetrics/VictoriaMetrics/pull/11548).
...
* FEATURE: support for accepting HTTP requests over Unix domain socket via `-httpListenAddr=unix:/path/to/socket` command-line flag. Note that `-tls` and `-httpListenAddr.useProxyProtocol` cannot be used with Unix domain sockets. See [#1618](https://github.com/VictoriaMetrics/VictoriaLogs/issues/1618).

* BUGFIX: [security](https://docs.victoriametrics.com/victorialogs/security-and-lb/): allow access to the `/delete/*`, `/internal/log_new_streams` and `/internal/partition/*` endpoints with only the corresponding `-deleteAuthKey`, `-logNewStreamsAuthKey` or `-partitionManageAuthKey`, even when `-httpAuth.*` is also configured, without additionally requiring `-httpAuth.*` credentials. See [#1764](https://github.com/VictoriaMetrics/VictoriaLogs/issues/1764).
* BUGFIX: [data ingestion](https://docs.victoriametrics.com/victorialogs/data-ingestion/): fix insert requests getting stuck after another insert request times out waiting for a free `-maxConcurrentInserts` slot. Previously, this caused clients to time out and memory usage to grow at `vlinsert`. See [#1743](https://github.com/VictoriaMetrics/VictoriaLogs/issues/1743).
